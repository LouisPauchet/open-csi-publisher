...
// doesn't have to).
//
// Wrapped in an IIFE: on the listing page this file loads alongside
// dataset_panel.js as a plain, non-module <script> tag, sharing the global
// scope — without this, both files' top-level `const BASE_PATH = ...` would
// collide ("Identifier 'BASE_PATH' has already been declared"), a
// SyntaxError that aborts the whole second script's execution and silently
// breaks whatever it defines (window.showDatasetPanel, in that case).
// Anything the other script needs to call is exposed explicitly via
// window.* below, same as before.
(function () {
